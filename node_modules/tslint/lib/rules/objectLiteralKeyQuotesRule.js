...
var ObjectLiteralKeyQuotesWalker = (function (_super) {
        ...
        switch (this.mode) {
            ...
            case "consistent-as-needed":
                if (properties.some(function (_a) {
                    var name = _a.name;
                    return name.kind === ts.SyntaxKind.StringLiteral && propertyNeedsQuotes(name.text);
                })) {
                    this.allMustHaveQuotes(properties);
                }
                else {
    ...
    ObjectLiteralKeyQuotesWalker.prototype.allMustHaveQuotes = function (properties) {
        for (var _i = 0, properties_1 = properties; _i < properties_1.length; _i++) {
            var name_1 = properties_1[_i].name;
            if (name_1.kind !== ts.SyntaxKind.StringLiteral && name_1.kind !== ts.SyntaxKind.ComputedPropertyName) {
                this.addFailureAtNode(name_1, Rule.UNQUOTED_PROPERTY(name_1.getText()));
            }
        }
    };
    ObjectLiteralKeyQuotesWalker.prototype.noneMayHaveQuotes = function (properties, noneNeedQuotes) {
        for (var _i = 0, properties_2 = properties; _i < properties_2.length; _i++) {
            var name_2 = properties_2[_i].name;
            if (name_2.kind === ts.SyntaxKind.StringLiteral && (noneNeedQuotes || !propertyNeedsQuotes(name_2.text))) {
                this.addFailureAtNode(name_2, Rule.UNNEEDED_QUOTES(name_2.text));
            }
        }
    };
...
function quotesAreInconsistent(properties) {
    ...
    for (var _i = 0, properties_3 = properties; _i < properties_3.length; _i++) {
        var kind = properties_3[_i].name.kind;
        if (kind === ts.SyntaxKind.ComputedPropertyName) {
            continue;
        }
        var thisOneIsQuoted = kind === ts.SyntaxKind.StringLiteral;
